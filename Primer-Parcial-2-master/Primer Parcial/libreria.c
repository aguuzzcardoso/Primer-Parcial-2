...
void setCliente(eCliente arrayClientes[], int indice, int id, char nombre[], char apellido[], char sexo, char telefono[], int idLocalidad)
{
    ...
    arrayClientes[indice].idLocalidad = idLocalidad;
}

void setLocalidad(eLocalidad arrayLocalidades[], int indice, int codigo, char localidad[])
{
    arrayLocalidades[indice].codigo = codigo;
    strcpy(arrayLocalidades[indice].localidad, localidad);
    arrayLocalidades[indice].isEmpty = 0;
}
...
void altaCliente (eCliente arrayClientes[], eLocalidad arrayLocalidades[])
{
    ...
    else
    {
        ...
        listarLocalidad(arrayLocalidades);

        int codigoLocalidad = getValidInt("Ingrese el codigo del juego : ","El codigo puede contener solamente numeros\n", 1, MAX);
        int indiceLocalidad = buscarLocalidad(arrayLocalidades,codigoLocalidad);

        if(indiceLocalidad != -1){
            setCliente(arrayClientes,indice,codigo,nombre,apellido,sexo,telefono,codigoLocalidad);
        }
    }
}
...
void modificarCliente(eCliente arrayClientes[], eLocalidad arrayLocalidades[]){
    ...
    if(indiceLocalidad != -1){
        setCliente(arrayClientes,index,codigo,nombre,apellido,sexo,telefono,codigoLocalidad);
    }
...
{
    int j;
    printf("---LISTA LOCALIDADES---\n");
    printf("Codigo   Localidad \n");

    for (j=0; j < MAX; j++)
    {
        if(arrayLocalidades[j].isEmpty == 0)
        {
            printf(" %i   %s\n", arrayLocalidades[j].codigo, arrayLocalidades[j].localidad);
        }
    }
    system("pause");
}
...
void listarJuegos (eJuego arrayJuegos[])
{
    ...
    for (j=0; j < MAX; j++)
    {
        ...
        {
            printf(" %i   %s\n", arrayJuegos[j].codigo, arrayJuegos[j].descripcion);
        }
    }
    system("pause");
}
...
void listarMujeres (eCliente arrayClientes[])
{
    int j;
    printf("---LISTA JUEGOS---\n");
    printf("Codigo   Clientes \n");

    for (j=0; j < MAX; j++)
    {
        if(arrayClientes[j].isEmpty == 0 && arrayClientes[j].sexo == "f" || "F")
        {
            printf(" %i   %s\n", arrayClientes[j].codigo, arrayClientes[j].nombre);
        }
    }
    system("pause");
}

int buscarLocalidad(eLocalidad arrayLocalidad[], int codigo)
{
    int i;
    for(i=0; i < MAX; i++)
    {
        if(arrayLocalidad[i].isEmpty == 0 && arrayLocalidad[i].codigo == codigo) {
            return i;
        }
    }
    printf("\n\nNO SE ENCUENTRA ESE CODIGO\n");
...
int buscarMujer(eCliente arrayCliente[], int codigo)
{
    int i;
    for(i=0; i < MAX; i++)
    {
        if(arrayCliente[i].isEmpty == 0 && arrayCliente[i].sexo == "f" || "F") {
            return i;
        }
    }
    printf("\n\nNO SE ENCUENTRA ESE CODIGO\n");
...
void listarClientesSegunJuego(eAlquiler arrayAlquileres[], eCliente arrayClientes[], eJuego arrayJuegos[])
{
    ...
    listarJuegos(arrayJuegos);

    int codigoJuego = getValidInt("Ingrese el codigo del juego : ","El codigo puede contener solamente numeros\n", 1, 20);
    int indiceJuego = buscarJuego(arrayJuegos,codigoJuego);
    ...
    if(indiceJuego != -1){
        for (i=0; i < MAX; i++)
        {
            for(j=0; j < MAX; j++)
            {
                if (codigoJuego == arrayAlquileres[j].codigoJuego && arrayClientes[i].codigo == arrayAlquileres[j].codigoJuego)
                {
                    hayResultado=1;
                    printf("El cliente %s solicito el juego.\n", arrayClientes[i].nombre);
                    break;
                }
            }
        }
    ...
    if (hayResultado == 0) {
        printf("El cliente no retiro juegos\n");
    }
    system("pause");
}
void listarClientesSegunLocalidad(eLocalidad arrayLocalidades[], eCliente arrayClientes[])
{
    ...
    int codigoLocalidad = getValidInt("Ingrese el codigo de localidad : ","El codigo puede contener solamente numeros\n", 1, 20);
    int indiceLocalidad = buscarLocalidad(arrayLocalidades,codigoLocalidad);

    if(indiceLocalidad != -1){
        for (i=0; i < MAX; i++)
        {
            if (codigoLocalidad == arrayClientes[i].idLocalidad)
            {
                hayResultado=1;
                printf("El cliente %s es de tal localidad.\n", arrayClientes[i].nombre);
                break;
            }
        }
    }
...
void listarAlquiladosPorMujeres(eAlquiler arrayAlquileres[], eCliente arrayClientes[])
{
    ...
    if(indiceMujer != -1){
        for (i=0; i < MAX; i++)
        {
            if (codigoMujer == arrayClientes[i].idLocalidad)
            {
                hayResultado=1;
                printf("El cliente %s es de tal localidad.\n", arrayClientes[i].nombre);
                break;
